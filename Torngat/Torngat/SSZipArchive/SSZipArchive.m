      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    do {
        ...
        @autoreleasepool {
            ...
            if ([strPath containsString:@".."]) {
                unzCloseCurrentFile(zip);
                ret = unzGoToNextFile(zip);
                free(filename);
                continue;
            }
            if ([strPath hasPrefix:@"/"]) {
                unzCloseCurrentFile(zip);
                ret = unzGoToNextFile(zip);
                free(filename);
                continue;
            }
            if ([[[[NSFileManager defaultManager] attributesOfItemAtPath:strPath error:nil] fileType] isEqual:NSFileTypeSymbolicLink]) {
                unzCloseCurrentFile(zip);
                ret = unzGoToNextFile(zip);
                free(filename);
                continue;
